...
```json
{
  "query": {
    "bool": {
      "must": [{ "match": { "claim": "metarrestin" } }],
      "filter": [{ "term": { "jurisdiction": "US" } }]
    }
  },
  "size": 10,
...
```json
{
  "query": {
    "bool": {
      "must": [{ "match": { "full_text": "perinucleolar compartment inhibitor" } }],
      "filter": [{ "term": { "jurisdiction": "US" } }, { "range": { "year_published": { "gte": 2015 } } }]
    }
  }
}
```
...
- **Keep BigQuery `patent-search`** — Lens replaces the _need_ for BigQuery full-text, but don't remove it; fix BigQuery separately if desired
